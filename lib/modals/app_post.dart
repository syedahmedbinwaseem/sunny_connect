...
class Post{
  ...
  static Future<bool> addNewPost(String title,String userId, String classId, String fileType, String fileUrl, String postType, bool isStream, String fileExtension)async{
    try{
      DocumentReference doc= FirebaseFirestore.instance.collection(DatabaseStrings.POST_COL).doc();
      doc.set(
        {
          'id' : doc.id,
          'title' : title,
  ...
  static Stream<QuerySnapshot<Map<String, dynamic>>> streamPostWithClassId(String id){
    return FirebaseFirestore.instance.collection(DatabaseStrings.POST_COL).where('class_id', isEqualTo: id).snapshots();
  }

  static Future<bool> deletePost(String id)async{
    try{
      await FirebaseFirestore.instance.collection(DatabaseStrings.POST_COL).doc(id).delete();
      return true;
    }catch(e){
      Fluttertoast.showToast(msg: 'Error ${e.code}: ${e.message}');
      return false;
